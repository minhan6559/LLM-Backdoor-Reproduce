...
def train(config, **kwargs):
    ...
    logger = get_logger("train", os.path.join(config.output_dir, config.log_file))
    logger.info(config)
    ...
    pet, _, mlm = get_pet_mappers(
        tokenizer, reader, model, device, config.pet_method, config.mask_rate
    )
    ...
    global_step, best_score, early_stop_count = 0, -1.0, 0
    ...
    for epoch in range(1, config.max_train_epochs + 1):
        ...
        for step, batch in iterator:
            ...
            do_update = (
                global_step % config.grad_acc_steps == 0
            ...
            if global_step % config.eval_every_steps == 0:
                ...
                for name, loader in [
                    ...
                    assert (
                        config.save_metric in scores
                    ), f"Invalid metric name {config.save_metric}"
...
def train_adaptive_attack(
    config, model: AutoModelForMaskedLM, tokenizer: AutoTokenizer
):
    ...
    for epoch in range(1, config.max_train_epochs + 1):
        ...
        for step, batch in iterator:
            ...
            # adaptive loss
            pet, _, mpet = get_pet_mappers(
                tokenizer, reader, model, device, config.pet_method, config.mask_rate
            )

            masked_logits = []
            for _ in range(10):
                logits, _, _, _ = get_logits(
                    model, mpet, config, train_loader, format="torch"
                )
...
def test_backdoor(config, model, tokenizer, **kwargs):
    ...
    config.update(kwargs)
    logger = get_logger(
        "backdoor test", os.path.join(config.output_dir, config.log_file)
    )
    logger.info(config)
    device = torch.device("cuda:0" if config.use_gpu else "cpu")
...
def get_logits(model, pet, config, dataloader, format: str = "numpy"):
    ...
    for i, batch in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            pet.forward_step(batch)
            logits, ys, preds, pred_logits = pet.get_logits(
                batch, config.full_vocab_loss
            )
        # t2 = time.time()
        # mean_time += t2-t1
...
def run_test(model, pet, mpet, config, data_loader, anchor_logits, subset: str = None):
    ...
    return (
        np.nanmean(weights, axis=1)[ys == preds],
        np.nanstd(coef, axis=1)[ys == preds],
    )
...
def detect_backdoor(config, model, tokenizer, **kwargs):
    ...
    logger = get_logger(
        "backdoor detection", os.path.join(config.output_dir, config.log_file)
    )
    logger.info(config)
    device = torch.device("cuda:0" if config.use_gpu else "cpu")
    ...
    clean_wt, clean_coef = run_test(
        model, pet, mpet, config, test_loader, anchor_logits, "clean"
    )
    if config.is_test:
    ...
    poison_wt, poison_coef = run_test(
        model, pet, mpet, config, poison_loader, anchor_logits, "poison"
    )
    if config.is_test:
    ...
    if not config.is_test:
        ...
        np.save(
            os.path.join(config.output_dir, "coef.npy"),
        ...
        print("frr", frr[(frr >= 0) & (frr <= 0.15)])
        np.save(
            os.path.join(config.output_dir, "wt.npy"),
            {"far": far, "frr": frr, "auc": wauc},
        )
